...
def get_cb_contacts(gapped_cb_lst):
    ...
    for i, cb1 in enumerate(gapped_cb_lst):
        # print(cb1)
        if not len(cb1) > 1 and cb1 == '-':
            continue
        for j, cb2 in enumerate(gapped_cb_lst):
            if not len(cb2) > 1 and cb2 == '-':
                continue
            diff_vec = cb1 - cb2
...
def get_ppvs(contacts_x, contacts_y, ref_contact_map,
    ...
    for num_c in range(min(len(contacts_x),
                           math.ceil(ref_len * factor)) + 1)[1:]:
        TP = 0.0
        FP = 0.0
...
def plot_map(fasta_filename, c_filename, factor, c2_filename='', ss_fname='',
             psipred_horiz_fname='', psipred_vert_fname='', pdb_filename='',
             is_heavy=False, chain='', sep=',', outfilename=''):

    acc = fasta_filename.split('.')[0][:4]

    # get sequence
    seq = list(parse_fasta.read_fasta(open(fasta_filename, 'r'))
               .values())[0][0]
    ref_len = len(seq)
    ...
    if pdb_filename:
        ...
        ax.scatter(ref_contacts_x, ref_contacts_y, marker='o', c='#CCCCCC',
                   lw=0, edgecolor='#CCCCCC')
    ...
    if c2_filename:
        ...
        if pdb_filename:
            ...
            tp2_colors = get_tp_colors(contacts2_x, contacts2_y,
                                       ref_contact_map, atom_seq_ali)
            print('%s %s %s %s' % (pdb_filename, PPVs2[-1],
                                   TPs2[-1], FPs2[-1]))
            fig.suptitle('%s\nPPV (upper left) = %.2f |' % (PPVs[-1]) +
                         'PPV (lower right) = %.2f' % (PPVs2[-1]))
            sc = ax.scatter(contacts2_y[::-1], contacts2_x[::-1], marker='o',
                            c=tp2_colors[::-1], s=6, alpha=0.75,
                            linewidths=0.0)
            sc = ax.scatter(contacts_x[::-1], contacts_y[::-1], marker='o',
                            c=tp_colors[::-1], s=6, alpha=0.75,
                            linewidths=0.0)
        else:
            sc = ax.scatter(contacts2_y[::-1], contacts2_x[::-1], marker='o',
                            c='#D70909', edgecolor='#D70909', s=4,
                            linewidths=0.5)
            sc = ax.scatter(contacts_x[::-1], contacts_y[::-1], marker='o',
                            c='#004F9D', edgecolor='#004F9D', s=4,
                            linewidths=0.5)
    ...
    # if no second contact map given
        ...
        if pdb_filename:
            ...
            sc = ax.scatter(contacts_x[::-1], contacts_y[::-1], marker='o',
                            c=tp_colors[::-1], s=6, alpha=0.75,
                            linewidths=0.0)
            sc = ax.scatter(contacts_y[::-1], contacts_x[::-1], marker='o',
                            c=tp_colors[::-1], s=6, alpha=0.75,
                            linewidths=0.0)
        else:
            sc = ax.scatter(contacts_x[::-1], contacts_y[::-1], marker='o',
                            c=scores[::-1], s=4, alpha=0.75, cmap=cm.jet,
                            linewidths=0.1)
            sc = ax.scatter(contacts_y[::-1], contacts_x[::-1], marker='o',
                            c=scores[::-1], s=4, alpha=0.75, cmap=cm.jet,
                            linewidths=0.1)
